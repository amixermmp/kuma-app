...
export async function GET(request: NextRequest) {
  ...
  // ราคามาตรฐาน + โปร "จ่ายกี่วัน จาก 7 วัน" ตั้งได้เฉพาะต่อสาขาผ่าน branch_model_pricing — ไม่ตั้งไว้ = ค่ากลาง
  ...
  const getPromoDays = (branchId: string | null, brand: string, model: string) =>
    pricingMap.get(`${branchId ?? ''}__${brand}__${model}`)?.promoPayDays ?? 5
  ...
  const bikesWithPromo = bikes.map(b => ({
    ...resolveBikeRateFromMap(b, pricingMap),
    promo_pay_days: getPromoDays(b.branch_id, b.brand, b.model),
  }))
  ...
  return NextResponse.json({ bikes: result, outOfStockByBranch, posterDataByBranch })
}
