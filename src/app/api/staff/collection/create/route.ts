...
export async function POST(request: NextRequest) {
  ...
  const { data: rental } = await supabase
    ...
    .select('monthly_rate, swap_log, bikes(license_plate), customers(name)')
  ...
  // ถ้าเคยสลับรถหลังรอบนี้กำหนดชำระไปแล้ว ใช้ราคาเดิมตอนนั้น ไม่ใช่ราคาใหม่ที่อาจแพงกว่า (กันเก็บเกินย้อนหลัง)
  const applicableRate = getApplicableMonthlyRate(rental.monthly_rate, rental.swap_log, dueDate)
  ...
  // Mark all pending/overdue records up to this due_date as paid (clears stale alerts)
  if (status === 'paid') {
