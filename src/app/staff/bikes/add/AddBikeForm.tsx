  ...
  return (
    <div className="app-wrap">
      ...
      <div className="section-pad" style={{ paddingTop: '12px' }}>
        ...
        {/* รูปรถ */}
        <div className="card-title" style={{ padding: '4px 0 8px' }}>รูปรถ</div>
        <div className="card">
          <PhotoUpload
            icon="🛵"
            hint="ถ่ายหรือเลือกรูปรถ"
            folder="bikes"
        ...
        <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '8px' }}>
          ลูกค้าดูผ่าน QR ได้ — ดาวน์โหลดไม่ได้
        </div>

        {/* หน้าเล่มรถ */}
        <div className="card" style={{ marginBottom: '8px' }}>
          <div style={{ padding: '10px 14px 4px', fontSize: '13px', fontWeight: 600, color: '#374151' }}>
            📗 หน้าเล่มรถ
          </div>
          <div style={{ padding: '0 14px 12px' }}>
            <PhotoUpload
              icon="📗"
              hint="ถ่ายหรืออัพโหลดหน้าเล่มรถ"
              folder="docs/registration"
              onUpload={url => setRegPhotoUrl(url)}
              onRemove={() => setRegPhotoUrl('')}
            />
          </div>
        </div>
          ...
          <div style={{ padding: '0 14px 0' }}>
            <div className="field-row">
              ...
              <input className="field-input" type="date"
                value={taxExpiry} onChange={e => setTaxExpiry(e.target.value)} />
            </div>
            <div className="field-row" style={{ marginBottom: '12px' }}>
              ...
              <div style={{ flex: 1 }}>
                <PhotoUpload
                  icon="💰"
                  hint="ถ่ายป้ายภาษีประจำปี"
                  folder="docs/tax"
                  onUpload={url => setTaxPhotoUrl(url)}
                  onRemove={() => setTaxPhotoUrl('')}
                />
              </div>
            </div>
          </div>
        ...
        <div className="card" style={{ marginBottom: '8px' }}>
          ...
          <div style={{ padding: '0 14px 0' }}>
            <div className="field-row">
              <label className="field-label">วันหมดอายุ</label>
              <input className="field-input" type="date"
                value={pobExpiry} onChange={e => setPobExpiry(e.target.value)} />
            </div>
            <div className="field-row" style={{ marginBottom: '12px' }}>
              <label className="field-label">อัพโหลดรูป</label>
              <div style={{ flex: 1 }}>
                <PhotoUpload
                  ...
                  hint="ถ่ายใบ พ.ร.บ."
                  folder="docs/pob"
                  onUpload={url => setPobPhotoUrl(url)}
                  onRemove={() => setPobPhotoUrl('')}
                />
              </div>
            </div>
          </div>
        ...
        <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '8px' }}>
          เตือนเมื่อถึงกิโลเมตร หรือ จำนวนวัน อย่างใดอย่างหนึ่งก่อน
        </div>
            ...
            <div style={{ padding: '10px 14px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              {i >= 2 && (
                <button onClick={() => removeRoutine(i)}
                  style={{ color: '#dc2626', background: 'none', border: 'none', fontSize: '18px', cursor: 'pointer', padding: '0 4px' }}>
                  ×
                </button>
              )}
            </div>
