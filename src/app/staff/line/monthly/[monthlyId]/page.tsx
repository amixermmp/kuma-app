...
import LineLinkClient from '../../[rentalId]/LineLinkClient'

export const dynamic = 'force-dynamic'

// ผูกไลน์ลูกค้าเช่ารายเดือน
export default async function StaffLineLinkMonthlyPage({ params }: { params: { monthlyId: string } }) {
  ...
  const { data: monthly } = await supabase
    ...
    .select('id, branch_id, customer_id, payment_day, customers(name, phone), bikes(id, license_plate, brand, model)')
    .eq('id', params.monthlyId)
    .eq('status', 'active')
    .single()

  if (!monthly) redirect('/staff/home')

  const [{ data: settings }, { data: link }] = await Promise.all([
    supabase
      .from('branch_settings')
      .select('line_liff_id')
      .eq('branch_id', monthly.branch_id)
      .maybeSingle(),
    supabase
      .from('customer_line_links')
      .select('id')
      .eq('customer_id', monthly.customer_id)
      .eq('branch_id', monthly.branch_id)
      .maybeSingle(),
  ])
  ...
  const liffUrl = `https://liff.line.me/${settings.line_liff_id}?monthly=${monthly.id}`
      ...
      <div className="section-pad" style={{ paddingTop: '20px', textAlign: 'center' }}>
        ...
        <div className="card" style={{ padding: '24px', alignItems: 'center', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div style={{ fontSize: '13px', color: '#6b7280' }}>
            {bike?.brand} {bike?.model} • {bike?.license_plate} • จ่ายทุกวันที่ {monthly.payment_day}
          </div>
        </div>

        <div style={{ marginTop: '14px' }}>
          <LineLinkClient statusQuery={`monthlyId=${monthly.id}`} initialLinked={Boolean(link)} />
        </div>

        <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '14px' }}>
          ผูกแล้วระบบจะแจ้งเตือนลูกค้าทางไลน์อัตโนมัติ<br />
          เมื่อถึงกำหนดชำระค่าเช่ารายเดือน และเมื่อถึงรอบเซอร์วิสรถ
        </div>
