...
export default async function SwapPage({
  ...
  if (rentalType === 'monthly') {
    const { data, error } = await supabase
      ...
      .select('id, bike_id, branch_id, monthly_rate, payment_day, swap_log, bikes(id, license_plate, brand, model), customers(name, phone)')
      .eq('id', rentalId)
      .eq('status', 'active')
      .single()
    ...
    const { data, error } = await supabase
      .from('rentals')
      .select('id, bike_id, branch_id, daily_rate, total_days, expected_end_datetime, bikes(id, license_plate, brand, model), customers(name, phone)')
      .eq('id', rentalId)
  ...
  const currentBikeId = (rental.bikes as { id: string }).id

  // ── Available bikes (same branch, not current) ──────────────────────────────
