...
export default async function BikesPage() {
  ...
  const addBtn = (
    <Link href="/bikes/new" style={{ color: '#f59e0b', fontWeight: 600, fontSize: '15px', textDecoration: 'none' }}>
      + เพิ่มรถ
    </Link>
  )
          ...
          <Link key={bike.id} href={`/bikes/${bike.id}`} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <p style={{ color: '#374151', fontSize: '13px', marginTop: '4px', fontWeight: 500 }}>
                  {Number(bike.daily_rate).toLocaleString()} บ./วัน
                  {bike.monthly_rate ? ` · ${Number(bike.monthly_rate).toLocaleString()} บ./เดือน` : ''}
                </p>
              </div>
              <span style={{
                fontSize: '12px', fontWeight: 700,
                padding: '5px 12px', borderRadius: '20px',
                background: (statusColor[bike.status] ?? '#6b7280') + '20',
                color: statusColor[bike.status] ?? '#6b7280',
              }}>
