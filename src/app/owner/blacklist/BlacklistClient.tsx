...
type Entry = { id: string; name: string; phone: string | null; id_card_number: string | null; reason: string | null; created_at: string }
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch('/api/owner/blacklist', {
        ...
        body: JSON.stringify({ name, phone, idCardNumber, reason }),
      })
      const data = await res.json()
      if (!res.ok) { setError(data.error || 'เกิดข้อผิดพลาด'); return }
      setName(''); setPhone(''); setIdCardNumber(''); setReason('')
      router.refresh()
    } catch {
