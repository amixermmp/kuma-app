...
type BranchLine = { branch_id: string; line_token: string | null; line_liff_id: string | null; promptpay_id: string | null; line_notify_customer: boolean | null; frame_url: string | null; sticker_url: string | null }
...
// ─── BranchMarketingCard (กรอบ+สติ๊กเกอร์ปิดหน้า รูปคู่รถ รายสาขา) ─────────────

function BranchMarketingCard({ branch, initial }: { branch: Branch; initial?: BranchLine }) {
...
export default function SettingsClient({ shop, staff: initialStaff, branches: initialBranches, promotions: initialPromos, branchId, branchDocs, branchLine }: Props) {
  ...
  // ── Toggle student promo — มีได้แค่โปรเดียว ปลดตัวเก่าให้อัตโนมัติ ──
  ...
  return (
    <div style={{ paddingBottom: '40px' }}>
      ...
      {/* ── กรอบ+สติ๊กเกอร์ปิดหน้า รูปคู่รถ (รายสาขา) ── */}
      <Section title="🖼️ รูปคู่รถ — กรอบ/สติ๊กเกอร์ (รายสาขา)">
        <div style={{ margin: '12px 16px 0', background: '#f0fdf4', borderRadius: '10px', padding: '12px', fontSize: '12px', color: '#166534', border: '1px solid #bbf7d0' }}>
        ...
        <div style={{ padding: '12px 16px' }}>
          {branches.map(b => (
            <BranchMarketingCard key={b.id} branch={b} initial={branchLine.find(l => l.branch_id === b.id)} />
          ))}
        </div>
      </Section>
