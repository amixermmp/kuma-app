...
export default async function ShopSettingsPage() {
  ...
  const [shopRes, branchRes, branchSettingsRes] = await Promise.all([
    ...
    admin.from('branch_settings').select('branch_id, close_time_earliest, payment_qr_daily_url, payment_qr_monthly_url, line_qr_url, line_id, whatsapp_qr_url, whatsapp_number, receipt_shop_name, receipt_address, receipt_phone, receipt_logo_url, student_promo_university, document_theme'),
  ])
  ...
  const branches = (branchRes.data ?? []).map(b => {
    ...
    return {
      ...
      lineQrUrl: s?.line_qr_url ?? null,
      lineId: s?.line_id ?? null,
      whatsappQrUrl: s?.whatsapp_qr_url ?? null,
      whatsappNumber: s?.whatsapp_number ?? null,
      receiptShopName: s?.receipt_shop_name ?? null,
      ...
      documentTheme: s?.document_theme ?? 'kuma',
    }
  })
