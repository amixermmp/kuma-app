        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {upcoming.map(r => {
              ...
              // Supabase returns joined rows as object (FK many-to-one)
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <p style={{ fontWeight: 700, fontSize: '14px' }}>
                        {bike?.license_plate} <span style={{ fontWeight: 400, color: '#6b7280' }}>{bike?.brand} {bike?.model}</span>
                      </p>
                    ...
                    <span style={{
                      ...
                      {overdue
                        ? `เกิน ${Math.floor(Math.abs(diffHrs) / 24)}วัน`
                        : diffHrs < 24 ? `${diffHrs}ชม.`
                        : `${Math.floor(diffHrs / 24)}วัน`}
                    </span>
                  </div>
                </Link>
              )
