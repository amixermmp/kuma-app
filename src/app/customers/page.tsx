...
export default async function CustomersPage() {
  ...
  const addBtn = (
    <Link href="/customers/new" style={{ color: '#f59e0b', fontWeight: 600, fontSize: '15px', textDecoration: 'none' }}>
      + เพิ่มลูกค้า
    </Link>
  )
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {customers?.map(c => (
          <Link key={c.id} href={`/customers/${c.id}`} style={{
            ...
            borderLeft: c.is_blacklisted ? '4px solid #ef4444' : '4px solid transparent',
          }}>
