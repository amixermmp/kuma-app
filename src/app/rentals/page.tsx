...
export default async function RentalsPage() {
  ...
  const addBtn = (
    <Link href="/rentals/new" style={{ color: '#f59e0b', fontWeight: 600, fontSize: '15px', textDecoration: 'none' }}>
      + เช่าใหม่
    </Link>
  )
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <span style={{
                    ...
                    color: overdue ? '#ef4444' : '#d97706',
                  }}>
                    {overdue
                      ? `เกิน ${Math.floor(Math.abs(diffHrs) / 24)}วัน ${Math.abs(diffHrs) % 24}ชม.`
                      : diffHrs < 24 ? `เหลือ ${diffHrs}ชม.`
                      : `เหลือ ${Math.floor(diffHrs / 24)}วัน`}
                  </span>
                  <p style={{ fontSize: '13px', fontWeight: 600, color: '#374151' }}>
                    {Number(r.daily_rate).toLocaleString()} บ./วัน
                  </p>
                </div>
              </div>
            </Link>
